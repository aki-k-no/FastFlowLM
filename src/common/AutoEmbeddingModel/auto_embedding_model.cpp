...
void AutoEmbeddingModel::_shared_load_model(std::string model_path, json model_info, bool enable_preemption) {
    ...
    this->embedding_model_ = std::make_unique<gemma_embedding>(*this->lm_config, this->npu.get());

    this->q4nx = std::make_unique<Q4NX>(this->model_path);

    this->embedding_model_->load_weights(*this->q4nx);

    this->q4nx.reset();
}

buffer<bf16> AutoEmbeddingModel::_shared_embed(std::vector<int>& tokens) {
    buffer<bf16> y = this->embedding_model_->embed(tokens);
    return y;
}
